
const helpers = require('./helpers')

/**
 * fixed direction (first letter of) North, East, South, West
 * @type {[string,string,string,string]}
 */
const directions = ['N', 'E', 'S', 'W']

/**
 * Rover Class
 * @type {Rover}
 */
const rover = class Rover {

  /**
   * Deploys the Rover on the given zone (x, y), facing the given direction (N, E, S or W)
   *
   * @param y Number
   * @param x Number
   * @param direction String
   */
  constructor (x, y, direction) {
    if (typeof x !== 'number' || typeof y !== 'number' || parseInt(x) < 1 || parseInt(x) < 1) {
      throw new TypeError(`Expected x & y to be numbers, got x: ${typeof x} & y: ${typeof y}`);
    }
    if (typeof direction !== 'string' || (direction !== 'N' && direction !== 'E' && direction !== 'S' && direction !== 'W')) {
      throw new TypeError(`Expected direction to be string (N, E, S or W), got type ${typeof direction} with value of ${direction}`);
    }
    this._x = x
    this._y = y
    this._direction = direction
  }

  /**
   * Calculates the distances between a list of nodes
   * @param instructionsString
   */
  command (instructionsString) {
    const instructions = instructionsString.split('')
    return helpers.eachSeries(instructions, instruction => {
      if (instruction === 'M') {
        return this.move()
      } else {
        return this.rotate(instruction)
      }
    })
  }

  /**
   * Rotate the Rover in the directions L = left and R = right
   * @param rotate (L, R)
   * @returns {Promise}
   */
  rotate (rotate) {
    return new Promise((resolve) => {
      // simulate async rotation
      setTimeout(() => {
        const rotateDir = (rotate === 'L') ? -1 : 1

        // get the correct
        let dir = directions.indexOf(this._direction) + rotateDir
        if (dir < 0) {
          dir = directions.length - 1
        } else if (dir >= directions.length) {
          dir = 0
        }
        this._direction = directions[dir]
        resolve()
      }, 50)
    })
  }

  /**
   * Move to rover one slot in the direction that it is facing right now
   * @returns {Promise}
   */
  move () {
    return new Promise((resolve) => {
      // simulate async rotation
      setTimeout(() => {
        switch (this._direction) {
          case 'N':
            this._y++
            break;
          case 'E':
            this._x++
            break;
          case 'S':
            this._y--
            break;
          case 'W':
            this._x--
            break;
        }
        resolve()
      }, 50)
    })
  }

  /**
   * Return the directions as "x y direction"
   * @returns {string}
   */
  getCoordinates () {
    return `${this._x} ${this._y} ${this._direction}`
  }

  /**
   * Returns the x coordinate
   * @returns {number}
   */
  getX () {
    return this._x
  }

  /**
   * Returns the y coordinate
   * @returns {number}
   */
  getY () {
    return this._y
  }

  /**
   * Returns the current direction (N, E, S, W)
   * @returns {string}
   */
  getDirection () {
    return this._direction
  }
}
module.exports = rover
